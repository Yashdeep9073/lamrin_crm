                ...
                <div class="col-md-12 col-lg-8">
                    <div class="card">
                        ...
                        <div class="card-block">
                            ...
                            <a href="{{ route($route . '.edit', $role->id) }}" class="btn btn-info"><i
                                    class="fas fa-sync-alt"></i> {{ __('btn_refresh') }}</a>
                        </div>

                        <form class="needs-validation" novalidate action="{{ route($route . '.update', [$role->id]) }}"
                            ...
                            <div class="card-block">
                                ...
                                <div class="form-group mb-4">
                                    ...
                                    <button type="button" class="btn btn-outline-secondary btn-sm" id="unselect-all">Unselect All</button>
                                </div>
                                ...
                                @foreach ($permission as $value)
                                    ...
                                    <div class="form-group d-inline" style="margin-right: 40px;">
                                        <div class="checkbox d-inline">
                                            <input type="checkbox" class="permission-checkbox" id="checkbox-{{ $value->id }}" name="permission[]"
                                                value="{{ $value->id }}"
                                                @foreach ($rolePermissions as $rolePermission)
                                    @if ($rolePermission->permission_id == $value->id) checked @endif @endforeach>

                                            <label for="checkbox-{{ $value->id }}"
                                                class="cr">{{ $value->title }}</label>
                                        </div>
                                    </div>
