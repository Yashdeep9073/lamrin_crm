                                        ...
                                        <select class="form-control" name="category" id="category">
                                            ...
                                            @foreach ($categories as $category)
                                                <option value="{{ $category->id }}"
                                                    @if ($selected_category == $category->id) selected @endif>{{ $category->title }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="table-responsive">
                                    <table id="export-table" class="display table nowrap table-striped table-hover"
                                        ...
                                        <tbody>
                                            @foreach ($rows as $key => $row)
                                                <tr>
                                                    ...
                                                    {{-- <td>{{ str_pad($row->id, 6, '0', STR_PAD_LEFT) }}</td> --}}
                                                     @php
                                                        $programTitle = $row->studentEnroll->program->title ?? '';
                                                        $programShort = collect(explode(' ', $programTitle))->map(fn($word) => strtoupper(substr($word, 0, 1)))->join('');
                                                        $receiptNumber = $programShort . '-' . str_pad($row->id, 6, '0', STR_PAD_LEFT);
                                                    @endphp
                                                    <td>{{ $receiptNumber }}</td>
                                                    <td>
                                            ...
                                            {{ number_format((float)$row->fees_total_amount, $setting->decimal_place, '.', '') }} 
                                            @else
                                            {{ number_format((float)$row->fees_total_amount, 2, '.', '') }} 
                                            @endif 
                                            ...
                                            {!! $setting->currency_symbol !!}
                                                    ...
                                                    <td>
                                                        ...
                                                        @elseif($row->status == 1)
                                                            ...
                                                            @can($access . '-action')
                                                                <button type="button" class="btn btn-icon btn-danger btn-sm"
                                                                    ...
                                                                    data-bs-target="#unpayModal-{{ $row->id }}">
                                                        ...
                                                        @endif
                                                        <a href="#" type="button"
                                                            class="viewButton btn btn-icon btn-primary btn-sm" data-bs-toggle="modal"
                                                            data-bs-target="#payModal-{{ $row->student_enroll_id }}"
                                                            data-id="{{ $row->student_enroll_id }}">
                                                            <i class="fas fa-eye"></i>
                                                        </a>

                                                        <!-- Include Pay modal -->
                                                        @include($view . '.fees-details')
                                                    </td>
                                                </tr>
                                            @endforeach
                                        </tbody>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            $('.viewButton').on('click', function (event) {
                ...
                if (id) {
                    $.ajax({
                        url: '/admin/fetch-fees/' + id + '/fees',
                        type: 'GET',
                        success: function(response) {
                            if (response) {
                                $('#totalFeeAmount').val(response.fees_total_amount);
                                $('#pendingFeeAmount').val(response.pending_fee_amount);
                                $('#feesDiscount').val(response.discount_amount);
