...
class ReportController extends Controller
{
    ...
    public function fees(Request $request)
    {
        ...
        $data['rows'] = $fees->whereIn('status', [1, 2])
            ->orderBy('updated_at', 'desc')
            ->get();
        // Calculate the sum of paid_amount for non-canceled fees (status != 2)
        $data['total_paid_amount'] = Fee::whereDate('pay_date', '>=', $start_date)
            ->whereDate('pay_date', '<=', $end_date)
            ->where('status', '!=', 2) // Exclude canceled status
            ->when($faculty != 0, function ($query) use ($faculty) {
                $query->whereHas('studentEnroll.program', function ($q) use ($faculty) {
                    $q->where('faculty_id', $faculty);
                });
            })
            ->when($program != 0, function ($query) use ($program) {
            ...
            ->when($section != 0, function ($query) use ($section) {
